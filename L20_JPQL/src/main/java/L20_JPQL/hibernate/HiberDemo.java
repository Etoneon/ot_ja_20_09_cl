package L20_JPQL.hibernate;

import L20_JPQL.hibernate.model.AddressDataSet;
...
import L20_JPQL.hibernate.model.Person;
import L20_JPQL.hibernate.model.Phone;
 ...
 //               .setProperty("hibernate.connection.url", URL)
    ...
    private void entityExample() {
        try (var session = sessionFactory.openSession()) {
            var person = new Person("Ivan", 42);
            var address = new AddressDataSet("Lenin street");
            var addressN = new AddressDataSet("Stalin street");
            var addressN2 = new AddressDataSet("Gorby street");
            var listPhone1 = createPhoneList(person, 1, 3);
            var listPhone2 = createPhoneList(person, 4, 6);
            var listPhone3 = createPhoneList(person, 7, 9);
 //           person.setName("Ivan");
 //           person.setAge(42);
            person.setAddress(address);
            person.setPhones(listPhone1);
            ...
            transaction.begin();
            person.setAddress(addressN);
//            person.setPhones(listPhone2); // удаление этого апдейта приводит в порядок updating detached
            transaction.commit();
            ...
            updated.setAge(53);
            updated.setAddress(addressN2);
            updated.setPhones(listPhone3);
    ...
    private ArrayList<Phone> createPhoneList(Person person, int begin, int end) {

        var listPhone = new ArrayList<Phone>();
        for (int idx = begin; idx <= end; idx++) {
            listPhone.add(new Phone("+"+ person.getName()+"_" + idx, person));
        }
         return listPhone;
    }

/*
    private long createPerson_n(Session session) {
 ...
 /*   private void entityExample1() {
    ...
    private void fetchExample() {
        long personId;
        try (var session = sessionFactory.openSession()) {
            personId = createPerson_o(session);
        }
    ...
    private long createPerson_o(Session session) {
        var transaction = session.getTransaction();
        transaction.begin();
